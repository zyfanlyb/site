...
import { MailOutlined, PhoneOutlined, WechatOutlined } from '@ant-design/icons-vue'

/**
 * 图标来源（二选一）：
 * 1) iconfont：在 .env 设置 VITE_ICONFONT_CSS=你的「Font class」在线 css 地址；
 ...
 * 微信：点击弹二维码。public/wechat-qr.png
 */
...
<template>
  <div class="home-contact-channels" role="group" aria-label="联系方式（图标入口）">
    <button
      type="button"
    ...
    >
      <span class="home-contact-channels__icon" aria-hidden="true">
        <i v-if="useIconfont" class="home-contact-channels__iconfont" :class="iconfontClasses(iconfontPhone)" />
        <PhoneOutlined v-else class="home-contact-channels__icon-antd" />
      </span>
      <span class="home-contact-channels__peek">{{ copyFlash === 'phone' ? '已复制' : CONTACT.phone }}</span>
    </button>
    ...
    >
      <span class="home-contact-channels__icon" aria-hidden="true">
        <i v-if="useIconfont" class="home-contact-channels__iconfont" :class="iconfontClasses(iconfontWechat)" />
    ...
    >
      <span class="home-contact-channels__icon" aria-hidden="true">
        <i v-if="useIconfont" class="home-contact-channels__iconfont" :class="iconfontClasses(iconfontMail)" />
        <MailOutlined v-else class="home-contact-channels__icon-antd" />
      </span>
      <span class="home-contact-channels__peek home-contact-channels__peek--email">{{
  ...
  <Teleport to="body">
    <div v-if="wechatOpen" class="home-contact-modal-root">
      <div class="home-contact-modal-mask" aria-hidden="true" @click="closeWechat" />
      <div
      ...
      >
        <div class="home-contact-modal-content" @click.stop>
          <div class="home-contact-modal-header">
            <span id="home-contact-modal-title" class="home-contact-modal-title">微信扫一扫</span>
          </div>
          <button type="button" class="home-contact-modal-close" aria-label="关闭" @click="closeWechat">
          ...
          <div class="home-contact-modal-body">
            <div class="home-contact-modal-body-inner">
              ...
              <p v-if="qrFailed" class="home-contact-modal-fallback">
                未找到二维码图片。请将文件放到 <code>public/wechat-qr.png</code> 后重新构建。
              </p>
            </div>
          </div>
        </div>
